    ...
    for fst_i in range(N):  # 첫 번째 일꾼의 i(행)
        for fst_j in range(N-M+1):  # 첫 번째 일꾼의 j(열), 꿀통은 열로 연속되어야 한다.
            ...
            for select_cnt in range(1, M+1):
                ...
                fst_comb_max = 0
                for c_list in comb:
                    # 주어진 조합들의 가치가 C를 넘어서면 안된다.
                    if sum(c_list) > C: continue
                    result = 0
                    for c in c_list:
                        result += (c ** 2)
                    fst_comb_max = max(fst_comb_max, result)
                fst_select_honey_max = max(fst_select_honey_max, fst_comb_max)
        # 두 번째 일꾼이 벌통을 골랐을 경우
            ...
            for snd_j in range(0, N-M+1):   # M 만큼 선택했을 때 인덱스가 겹치지 않게 설정
                ...
                for select_cnt in range(1, M + 1):
                    ...
                    for c_list in comb:
                        ...
                        for c in c_list:
                            result += (c ** 2)
                        snd_comb_max = max(snd_comb_max, result)
                    snd_select_honey_max = max(snd_select_honey_max, snd_comb_max)
